...
export async function POST(req: NextRequest) {
  try {
    ...
    const paymentResponse = await fetch(
      ...
      {
        headers: {
          Authorization: `PortOne apTgCI3T0g6zDOGTpzPANzu2Xyv8a6bHNefHCtdFS5g2ZECHWJOrRWtHXiVBXCxwuT8kNt5iW7bBF0NB`,
        },
      }
    );
    ...
    const orderResp = await axios
      .get(process.env.SPRING_API + "/api/order/valid?" + "orderId=" + orderId)
      .then((orderResp) => {
        ...
        if (
          orderResp.data.content.orderPrice !== payment.amount.total &&
          orderResp != null &&
          payment != null
        ) {
          return NextResponse.json(
    ...
    // 3. 성공 응답
    return NextResponse.json({ status: "PAID", paymentId }, { status: 200 });
  } catch (error) {
