...
export default function FeeTable({
  ...
  return (
    <div className="bg-[#1e293b]/80 shadow-md rounded-lg border border-blue-900 p-6 overflow-x-auto">
      {title && <h2 className="text-2xl font-semibold text-white mb-4 text-center">{title}</h2>}
          ...
          {Object.entries(groupedByName).map(([name, items]) =>
            items.map((item, idx) => (
              <tr key={`${name}-${idx}`} className="bg-[#1e293b]/50">
                {idx === 0 && (
                  <td rowSpan={items.length} className="px-4 py-2 border border-blue-900 text-center align-middle">
                    {name}
                  </td>
                )}
