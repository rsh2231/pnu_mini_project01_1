...
export default function Payment({
  ...
  onSuccess: (orderId: string) => void; // 결제 성공 시 orderId 전달
}) {
  ...
  const [selectedData, setSelectedData] = useState<any[]>(
    data.map((item: any) => item.furnitureList)
  );
  const [selectedKeys, setSelectedKeys] = useState<string[]>(
    sorteditmes.map(
      (i: any) =>
        `${i.furnitureList.index}_${i.furnitureList.품명}_${i.furnitureList.규격}`
    )
  );
    ...
    <div className="fixed inset-0 bg-black/60 flex justify-center items-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6 sm:p-8">
        ...
        <div className="mb-6 text-center text-gray-600 text-sm sm:text-base leading-relaxed">
          품목을 클릭하여{" "}
          <span className="font-semibold">선택하거나 해제</span>할 수 있습니다.
          <br />
